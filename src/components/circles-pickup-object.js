...
AFRAME.registerComponent('circles-pickup-object', {
  ...
  init: function () {
    ...
    CONTEXT_AF.lastClickTime = 0;

    CONTEXT_AF.clickHandler = function (event) {
      // debounce
      var now = Date.now();
      if (now - CONTEXT_AF.lastClickTime < 500) {
        console.log('Ignoring click - too soon after last click');
        return;
      }

      CONTEXT_AF.lastClickTime = now;
      event.stopPropagation();
      event.preventDefault();

      CONTEXT_AF.clickFunc(event);
    };

    CONTEXT_AF.el.addEventListener('click', CONTEXT_AF.clickHandler);
  },
  ...
  pickup: function (sendNetworkEvent, passedContext) {
    ...
    if (data.animate === true) {
      CONTEXT_AF.el.setAttribute('animation__cpo_position', {
        property: 'position', dur: (CIRCLES.UTILS.isTheSameXYZ(pickupPos, thisPos, SAME_DIFF) ? 0.0 : data.animateDurationMS),
        isRawProperty: true, to: pickupPos, easing: 'easeInOutQuad'
      });
      CONTEXT_AF.el.setAttribute('animation__cpo_rotation', {
        property: 'rotation', dur: (CIRCLES.UTILS.isTheSameXYZ(pickupRot, thisRot, SAME_DIFF) ? 0.0 : data.animateDurationMS),
        isRawProperty: true, to: pickupRot, easing: 'easeInOutQuad'
      });
      CONTEXT_AF.el.setAttribute('animation__cpo_scale', {
        property: 'scale', dur: (CIRCLES.UTILS.isTheSameXYZ(pickupSca, thisSca, SAME_DIFF) ? 0.0 : data.animateDurationMS),
  ...
  release: function (sendNetworkEvent, passedContext) {
    ...
    if (data.animate === true) {
      CONTEXT_AF.el.setAttribute('animation__cpo_position', {
        property: 'position', dur: (CIRCLES.UTILS.isTheSameXYZ(dropPos, thisPos, SAME_DIFF) ? 0.0 : data.animateDurationMS),
        isRawProperty: true, to: dropPos, easing: 'easeInOutQuad'
      });
      CONTEXT_AF.el.setAttribute('animation__cpo_rotation', {
        property: 'rotation', dur: (CIRCLES.UTILS.isTheSameXYZ(dropRot, thisRot, SAME_DIFF) ? 0.0 : data.animateDurationMS),
        isRawProperty: true, to: dropRot, easing: 'easeInOutQuad'
      });
      CONTEXT_AF.el.setAttribute('animation__cpo_scale', {
        property: 'scale', dur: (CIRCLES.UTILS.isTheSameXYZ(dropSca, thisSca, SAME_DIFF) ? 0.0 : data.animateDurationMS),
